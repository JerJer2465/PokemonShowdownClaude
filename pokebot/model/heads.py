...
class PolicyHead(nn.Module):
    ...
    def forward(self, actor_token: torch.Tensor, legal_mask: torch.Tensor) -> torch.Tensor:
        ...
        # CUDA-graph-safe masking: torch.where instead of clone + bool indexing.
        # If all actions illegal (shouldn't happen), treat all as legal.
        safe_legal = torch.where(
            legal_mask.sum(dim=-1, keepdim=True) > 0,
            legal_mask,
            torch.ones_like(legal_mask),
        )
        logits = logits.masked_fill(safe_legal == 0, -1e9)
